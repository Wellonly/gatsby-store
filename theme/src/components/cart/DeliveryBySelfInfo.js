...
export default function DeliveryBySelfInfo({offices, city, carrier, i18}) {
  const cityOffices = offices.reduce((acc,office) => {
    if (office.city_id === city.id) return [...acc, office];
    return acc;
  },[]);
  ...
  // console.log('...zv: DeliveryBySelfInfo:',cityOffices);
  return (
    <Paper ref={funcRef} style={{marginLeft:-25}}>
      ...
      <ExpansionPanel>
        ...
        <ExpansionPanelDetails>
          {
            cityOffices.map((office,i) => {
              return office.title === officeTitle ? (
                <Box key={i} >
